...
class PositionDetailManager {
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e) => {
            ...
            // Asignar permiso (desde el picker)
    ...
    async loadAppPermissions() {
        ...
        try {
            const [permsResponse, assignedResponse] = await Promise.all([
            ...
            if (permsResponse.ok) {
                ...
                const assignedResult = assignedResponse.ok ? await assignedResponse.json() : { data: [] };
    ...
    populatePermissionPicker(assignedPerms = []) {
        ...
        // Group permissions by module (2nd segment of code: app.MODULE.type.action)
        const groups = {};
