...
def get_or_404_app(db: Session, app_key: str) -> App:
    ...
    if not app:
        raise HTTPException(status_code=404, detail=f"App '{app_key}' no existe o está inactiva.")
    return app

def get_role_by_name(db: Session, role_name: str) -> Optional[Role]:
    return db.query(Role).filter_by(name=role_name).first()

def get_perm(db: Session, app_id: int, code: str) -> Optional[Permission]:
    return db.query(Permission).filter_by(app_id=app_id, code=code).first()

# ---------------------------
# CRUD de asignaciones
# ---------------------------

def grant_role(db: Session, user_id: int, app_key: str, role_name: str) -> bool:
    """Asigna un rol de app al usuario. True si creó, False si ya existía."""
    app = get_or_404_app(db, app_key)
    role = get_role_by_name(db, role_name)
    if not role:
        raise HTTPException(status_code=400, detail=f"Rol '{role_name}' no existe.")

    exists = db.query(UserAppRole).filter_by(
...
def revoke_role(db: Session, user_id: int, app_key: str, role_name: str) -> bool:
    ...
    if not role:
        return False
    q = db.query(UserAppRole).filter_by(
        user_id=user_id, app_id=app.id, role_id=role.id
    )
    if q.count() == 0:
        return False
...
def grant_perm(db: Session, user_id: int, app_key: str, perm_code: str, *, allow: bool = True) -> bool:
    ...
    if not perm:
        raise HTTPException(status_code=400, detail=f"Permiso '{perm_code}' no existe en app '{app_key}'.")

    exists = db.query(UserAppPerm).filter_by(
...
def revoke_perm(db: Session, user_id: int, app_key: str, perm_code: str) -> bool:
    ...
    if not perm:
        return False
    q = db.query(UserAppPerm).filter_by(
        user_id=user_id, app_id=app.id, perm_id=perm.id
    )
    if q.count() == 0:
        return False
...
def user_roles_via_positions(db: Session, user_id: int, app_key: str) -> Set[str]:
...
def user_perms_via_positions_direct(db: Session, user_id: int, app_key: str) -> Set[str]:
...
def user_perms_via_position_roles(db: Session, user_id: int, app_key: str) -> Set[str]:
...
def _get_users_with_roles_in_app(db: Session, app_key: str, role_names: list[str]) -> list[int]:
    """
    ...
    try:
        ...
        result = db.execute(combined_query).fetchall()
...
def _get_users_with_position(db: Session, position_codes: list[str]) -> list[int]:
    """
...
def user_roles_in_app(db: Session, user_id: int, app_key: str, include_positions: bool = True) -> Set[str]:
    """
    ...
    if include_positions:
        roles |= user_roles_via_positions(db, user_id, app_key)

    return roles

def user_direct_perms_in_app(db: Session, user_id: int, app_key: str, include_positions: bool = True) -> Set[str]:
    """
    ...
    if include_positions:
        perms |= user_perms_via_positions_direct(db, user_id, app_key)

    return perms

def perms_via_roles(db: Session, user_id: int, app_key: str, include_positions: bool = True) -> Set[str]:
    """
    ...
    if include_positions:
        perms |= user_perms_via_position_roles(db, user_id, app_key)
...
def effective_perms(db: Session, user_id: int, app_key: str, include_positions: bool = True) -> Dict[str, Iterable[str]]:
    ...
    if include_positions:
        result["roles_via_positions"] = sorted(list(user_roles_via_positions(db, user_id, app_key)))
        result["perms_via_positions_direct"] = sorted(list(user_perms_via_positions_direct(db, user_id, app_key)))
        result["perms_via_position_roles"] = sorted(list(user_perms_via_position_roles(db, user_id, app_key)))

    return result

def list_roles(db: Session) -> Iterable[str]:
    return [r[0] for r in db.query(Role.name).order_by(Role.name.asc()).all()]

def list_perms(db: Session, app_key: str) -> Iterable[Dict]:
    app = get_or_404_app(db, app_key)
    rows = (
        db.query(Permission.code, Permission.name, Permission.description)
...
def has_any_assignment(db: Session, user_id: int, app_key: str, include_positions: bool = True) -> bool:
    """
...
def get_user_active_positions(db: Session, user_id: int) -> list[dict]:
    ...
    positions = (
        db.query(Position, UserPosition)
...
def user_has_position(db: Session, user_id: int, position_codes: list[str]) -> bool:
