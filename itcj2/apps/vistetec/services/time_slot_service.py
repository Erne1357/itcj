...
def get_slot_by_id(db: Session, slot_id: int) -> Optional[TimeSlot]:
...
def get_slots_for_date_range(db: Session, start_date: date, end_date: date) -> dict:
...
def update_slot(db: Session, slot_id: int, user_id: int, **kwargs) -> TimeSlot:
    """Actualiza un slot existente (solo el creador)."""
    slot = db.query(TimeSlot).filter_by(id=slot_id, created_by_id=user_id).first()
    if not slot:
...
def cancel_slot(db: Session, slot_id: int, user_id: int) -> TimeSlot:
...
def signup_volunteer(db: Session, slot_id: int, volunteer_id: int) -> SlotVolunteer:
    ...
    existing = db.query(SlotVolunteer).filter_by(
...
def unsignup_volunteer(db: Session, slot_id: int, volunteer_id: int) -> bool:
    """Voluntario cancela su inscripción a un slot."""
    sv = db.query(SlotVolunteer).filter_by(
        slot_id=slot_id, volunteer_id=volunteer_id
    ).first()
    if not sv:
...
def get_volunteer_signups(
    db: Session, volunteer_id: int, include_past: bool = False
) -> list[TimeSlot]:
    """Retorna los slots donde el voluntario está inscrito."""
    query = db.query(TimeSlot).join(SlotVolunteer).filter(
...
def is_volunteer_signed_up(db: Session, slot_id: int, volunteer_id: int) -> bool:
