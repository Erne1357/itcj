...
(function () {
    ...
    const API_BASE = '/api/vistetec/v2/garments';
    ...
    async function loadGarmentData() {
        try {
            const res = await fetch(`/api/vistetec/v2/catalog/${GARMENT_ID}`);
            if (!res.ok) return;
            ...
            if (g.image_path) {
                imagePreview.src = `/api/vistetec/v2/garments/image/${g.image_path}`;
                imagePreview.classList.remove('d-none');
                imagePlaceholder.classList.add('d-none');
            }
        } catch (e) {
