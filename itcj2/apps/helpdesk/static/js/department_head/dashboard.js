...
;(function () {
    ...
    function init() {
        ...
        DEPARTMENT_NAME = root.dataset.departmentName;

        // Expose window funcs that ACTUALLY EXIST in this module
    ...
    function destroy() {
        ...
        // Remove stats-tab listener
        if (_statsTabHandler) {
            const statsTab = document.getElementById('stats-tab');
            if (statsTab) statsTab.removeEventListener('shown.bs.tab', _statsTabHandler);
            _statsTabHandler = null;
        }

        // Remove passwords-tab listener
        if (_passwordsTabHandler) {
            const passTab = document.getElementById('passwords-tab');
            if (passTab) passTab.removeEventListener('shown.bs.tab', _passwordsTabHandler);
        ...
        if (btn && _btnResetMyPasswordHandler) {
    ...
    async function initializeDashboard() {
        try {
            ...
            // Load stats tab lazy
            _statsTabHandler = loadStatistics;
            document.getElementById('stats-tab').addEventListener('shown.bs.tab', _statsTabHandler);
    ...
    function setupResetMyPassword() {
        ...
        _btnResetMyPasswordHandler = async () => {
            const ok = await HelpdeskUtils.confirmDialog(
        ...
        btn.addEventListener('click', _btnResetMyPasswordHandler);
    }
    ...
    function setupPasswordsTab() {
        ...
        _passwordsTabLoaded = false;
        _passwordsTabHandler = () => {
            if (!_passwordsTabLoaded) {
                loadSecretaries();
                _passwordsTabLoaded = true;
            }
        };
        tabEl.addEventListener('shown.bs.tab', _passwordsTabHandler);
        ...
        if (btnRefresh) {
            ...
            btnRefresh.addEventListener('click', _btnRefreshHandler);
        }
    ...
    // ==================== REGISTER MODULE ====================
    window.HelpdeskPage.page('department_head_dashboard', { init, destroy });
