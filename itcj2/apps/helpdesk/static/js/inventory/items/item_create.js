...
(function () {
    ...
    function init() {
        ...
        BULK_MODE = root && root.dataset.bulkMode === 'true';
    ...
    function setupBulkListeners() {
        ...
        if (bulkCategory) bulkCategory.addEventListener('change', handleBulkCategoryChange);

        const destinationType = document.getElementById('bulk-destination-type');
        if (destinationType) destinationType.addEventListener('change', handleDestinationTypeChange);

        const bulkDepartment = document.getElementById('bulk-department-id');
        if (bulkDepartment) bulkDepartment.addEventListener('change', handleBulkDepartmentChange);

        const bulkGroupDepartment = document.getElementById('bulk-group-department-id');
        if (bulkGroupDepartment) bulkGroupDepartment.addEventListener('change', handleBulkGroupDepartmentChange);

        const quantityField = document.getElementById('bulk-quantity');
        if (quantityField) quantityField.addEventListener('input', updateBulkPreview);
    ...
    async function loadCategories() {
        try {
            ...
            if (!response.ok) throw new Error(await fetchApiError(response, 'Error al cargar categorías'));
    ...
    async function loadDepartments() {
        try {
            ...
            if (!response.ok) throw new Error(await fetchApiError(response, 'Error al cargar departamentos'));
    ...
    async function loadDepartmentUsers(departmentId) {
        ...
        try {
            ...
            if (!response.ok) throw new Error(await fetchApiError(response, 'Error al cargar usuarios del departamento'));
            ...
            showError(`No se pudieron cargar los usuarios del departamento: ${error.message || 'Error desconocido'}`);
        }
    }
    ...
    function updateInventoryPreview(prefix) {
        ...
        document.getElementById('preview-inventory-number').textContent = `${prefix}-${currentYear}-####`;
    }
    ...
    function renderSpecFields(template) {
        ...
        Object.entries(template).forEach(([key, config]) => {
            container.insertAdjacentHTML('beforeend', createSpecField(key, config));
        });
    }
    ...
    function renderSpecsContextHelp() {
        ...
        if (!isComputer) { helpBox.style.display = 'none'; helpBox.innerHTML = ''; return; }
        helpBox.innerHTML = `<i class="fas fa-info-circle"></i>
    ...
    function applyComputerSpecRules() {
        ...
        if (isAllInOne) { integratedDisplayField.checked = true; integratedDisplayField.disabled = true; }
        else { integratedDisplayField.disabled = false; }
        if (displaySizeField) {
            const displaySizeGroup = displaySizeField.closest('.spec-field');
            if (displaySizeGroup) displaySizeGroup.style.display = integratedDisplayField.checked ? 'block' : 'none';
        }
    }
    ...
    function normalizeComputerSpecificationsByCategory(specifications, category) {
        ...
        if (isAllInOne) specifications.integrated_display = true;
        if (!specifications.integrated_display) delete specifications.display_size;
    }
        ...
        switch (config.type) {
            case 'text':
                return `<div class="form-group spec-field"><label for="${id}" class="${requiredClass}">${label}</label>
                    <input type="text" class="form-control" id="${id}" name="spec_${key}"
                    placeholder="${config.placeholder || ''}" ${required}></div>`;
            case 'number':
                return `<div class="form-group spec-field"><label for="${id}" class="${requiredClass}">${label}</label>
                    <input type="number" class="form-control" id="${id}" name="spec_${key}"
                    placeholder="${config.placeholder || ''}" min="${config.min || 0}" ${required}></div>`;
            case 'select': {
            ...
            case 'textarea':
                return `<div class="form-group spec-field"><label for="${id}" class="${requiredClass}">${label}</label>
                    ...
                    placeholder="${config.placeholder || ''}" ${required}></textarea></div>`;
            default:
                return '';
        }
    }

    function showSpecsSection() { document.getElementById('specs-section').style.display = 'block'; }
    ...
    async function loadActiveCampaign(deptId) {
        ...
        try {
            ...
            if (!campaigns.length) { section.style.display = 'none'; return; }

            campaigns.forEach(c => {
                const opt = document.createElement('option');
                opt.value = c.id;
                opt.textContent = c.title ? `${c.folio} — ${c.title}` : c.folio;
                select.appendChild(opt);
            });
    ...
    async function loadBulkActiveCampaign(deptId) {
        ...
        try {
            ...
            if (campaigns.length === 1) select.value = String(campaigns[0].id);
            section.style.display = 'block';
        } catch (_) { section.style.display = 'none'; }
    }
    ...
    const _STATUS_LABELS = {
        ACTIVE: 'Activo', PENDING_ASSIGNMENT: 'Pendiente', MAINTENANCE: 'Mantenimiento',
        DAMAGED: 'Dañado', RETIRED: 'Retirado', LOST: 'Extraviado',
    };
    ...
    async function searchPredecessorItems(q) {
        ...
        try {
            ...
            resultsEl.innerHTML = `<div class="text-center text-danger py-3 small">${err.message}</div>`;
        }
    }
    ...
    async function showPredecessorDetail(itemId) {
        ...
        try {
            ...
            emptyEl.innerHTML = `<div class="text-center text-danger py-4 small">
    ...
    function _renderPredecessorDetail(item) {
        ...
        if (item.specifications && Object.keys(item.specifications).length > 0) {
            const specItems = Object.entries(item.specifications)
                .filter(([, v]) => v !== null && v !== '')
                .map(([k, v]) => `<div class="pred-spec-item">
        ...
        const infoHtml = `<div class="pred-spec-grid mt-2">
            ${rows.map(([label, val]) => `<div class="pred-spec-item">
                <small class="text-muted d-block">${label}</small>
                <span>${val}</span></div>`).join('')}</div>`;
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            showError(`Error al registrar equipo: ${error.message || 'Error desconocido'}`);
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-save"></i> Registrar Equipo';
        }
    }
    ...
    function collectFormData() {
        ...
        if (currentCategory && currentCategory.requires_specs && currentCategory.spec_template) {
            ...
            Object.keys(currentCategory.spec_template).forEach(key => {
                ...
                if (field) {
                    if (field.type === 'checkbox') specifications[key] = field.checked;
                    else if (field.value) specifications[key] = field.value;
                }
            });
            normalizeComputerSpecifications(specifications);
            if (Object.keys(specifications).length > 0) formData.specifications = specifications;
        }
    ...
    async function loadBulkCategories() {
        if (allCategories.length > 0) { populateBulkCategories(); return; }
        try {
            const response = await fetch('/api/help-desk/v2/inventory/categories?active=true', {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
            });
    ...
    async function loadBulkDepartments() {
        if (allDepartments.length > 0) { populateBulkDepartments(); return; }
        try {
            const response = await fetch('/api/core/v2/departments?active=true', {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
            });
    ...
    async function loadGroups(departmentId = null) {
        try {
            ...
            if (!response.ok) throw new Error(await fetchApiError(response, 'Error al cargar grupos'));
            const result = await response.json();
            allGroups = result.data || [];
            populateBulkGroups();
        } catch (error) {
            console.error('Error cargando grupos:', error);
            showError(`No se pudieron cargar los grupos: ${error.message || 'Error desconocido'}`);
            allGroups = [];
            populateBulkGroups();
        }
    }
    ...
    function renderBulkSpecFields(template) {
        ...
        container.innerHTML = '';
        Object.entries(template).forEach(([key, config]) => {
            container.insertAdjacentHTML('beforeend', createBulkSpecField(key, config));
        });
    }
    ...
    function createBulkSpecField(key, config) {
        ...
        switch (config.type) {
            case 'text':
                return `<div class="form-group spec-field"><label for="${id}" class="${requiredClass}">${label}</label>
                    <input type="text" class="form-control" id="${id}" name="bulk_spec_${key}"
                    placeholder="${config.placeholder || ''}" ${required}></div>`;
            case 'number':
                return `<div class="form-group spec-field"><label for="${id}" class="${requiredClass}">${label}</label>
                    <input type="number" class="form-control" id="${id}" name="bulk_spec_${key}"
                    placeholder="${config.placeholder || ''}" min="${config.min || 0}" ${required}></div>`;
            case 'select': {
                const options = config.options || [];
                const optionsHtml = options.map(opt => `<option value="${opt}">${opt}</option>`).join('');
    ...
    function handleBulkDepartmentChange(e) {
        ...
        if (destinationType === 'department') loadBulkActiveCampaign(departmentId);
        updateBulkPreview();
    }

    function handleBulkGroupDepartmentChange(e) {
        const departmentId = e.target.value;
        if (departmentId) loadGroups(departmentId);
        else { allGroups = []; populateBulkGroups(); }
        updateBulkPreview();
    }
    ...
    function updateBulkPreview() {
        ...
        const summaryHtml = `
        <div class="mb-2"><strong>Cantidad:</strong> ${quantity} equipos</div>
    ...
    function collectBulkFormData() {
        ...
        if (currentBulkCategory && currentBulkCategory.requires_specs && currentBulkCategory.spec_template) {
            ...
            if (Object.keys(specifications).length > 0) baseData.specifications = specifications;
        }

        for (let i = 1; i <= quantity; i++) {
            const item = { supplier_serial: null, itcj_serial: null, id_tecnm: null };
            if (destinationType === 'department') {
                ...
                if (groupId) item.group_id = parseInt(groupId);
            }
    ...
    window.showPredecessorDetail = showPredecessorDetail;

    // Registrar en el controller
    window.HelpdeskPage.page('inventory_items_item_create', { init, destroy });
