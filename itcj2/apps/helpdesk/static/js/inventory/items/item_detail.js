...
(function () {
    ...
    // Refs para listeners de tab
    let _historyTabHandler = null;
    let _ticketsTabHandler = null;
    let _versionsTabHandler = null;

    // ==================== INIT / DESTROY ====================
    function init() {
        const root = document.querySelector('[data-hd-page]');
        ITEM_ID = root ? parseInt(root.dataset.itemId) : null;
    ...
    function destroy() {
        ...
        if (historyTab && _historyTabHandler) historyTab.removeEventListener('shown.bs.tab', _historyTabHandler);
        if (ticketsTab && _ticketsTabHandler) ticketsTab.removeEventListener('shown.bs.tab', _ticketsTabHandler);
        if (versionsTab && _versionsTabHandler) versionsTab.removeEventListener('shown.bs.tab', _versionsTabHandler);
        ...
        CAN_EDIT = false;
        IS_ADMIN = false;
        _historyTabHandler = null;
        _ticketsTabHandler = null;
        _versionsTabHandler = null;
    }
    ...
    function setupEventListeners() {
        ...
        _historyTabHandler = function () { if (itemHistory.length === 0) loadHistory(); };
        _ticketsTabHandler = function () { if (itemTickets.length === 0) loadTickets(); };
        _versionsTabHandler = function () { if (currentItem) loadVersionChain(); };

        if (historyTab) historyTab.addEventListener('shown.bs.tab', _historyTabHandler);
        if (ticketsTab) ticketsTab.addEventListener('shown.bs.tab', _ticketsTabHandler);
        if (versionsTab) versionsTab.addEventListener('shown.bs.tab', _versionsTabHandler);
    ...
    async function loadItemDetail() {
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 403) { showForbidden(); return; }
    ...
    async function handleAssignUser(e) {
        ...
        if (!userId) { showError('Debe seleccionar un usuario'); return; }
    ...
    async function unassignUser() {
        ...
        try {
            const response = await fetch('/api/help-desk/v2/inventory/assignments/unassign', {
                ...
                body: JSON.stringify({ item_id: ITEM_ID, notes: 'Equipo liberado desde vista de detalle' })
            });
    ...
    async function handleChangeStatus(e) {
        ...
        if (!newStatus || !notes) { showError('Complete todos los campos'); return; }
    ...
    async function handleDeactivate(e) {
        ...
        if (reason.length < 10) { showError('La razón debe tener al menos 10 caracteres'); return; }
    ...
    async function handleEditLockedField(e) {
        ...
        if (justification.length < 10) { showError('La justificación debe tener al menos 10 caracteres'); return; }
    ...
    // Exponer funciones usadas en onclick generado dinámicamente
    ...
    window.openEditLockedField = openEditLockedField;
    window.openLinkPredecessorModal = openLinkPredecessorModal;
    window.unlinkPredecessor = unlinkPredecessor;
    window.selectPredecessor = selectPredecessor;
    window.openEditSpecsModal = openEditSpecsModal;

    window.HelpdeskPage.page('inventory_items_item_detail', { init, destroy });
