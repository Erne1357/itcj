/**
 * Lista de Campañas de Inventario — migrada a componentes server-side + HTMX.
 * La tabla, los filtros y la paginación los rinde el servidor (ver
 * _campaigns_list_results.html + pages/inventory.py). Este módulo conserva SOLO
 * la lógica de cliente: navegación por clic de fila (morph) y el botón Limpiar de
 * la barra de filtros. Sin jQuery.
 */
(function () {
    'use strict';
    ...
    window.HelpdeskPage.page('inventory_campaigns_campaigns_list', {
        init() {
            const results = document.getElementById('hd-campaigns-results');

            _rowClickHandler = function (e) {
                // Los enlaces (ojo / validar) manejan su propia acción.
