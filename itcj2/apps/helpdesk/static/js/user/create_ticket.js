...
// Módulo HTMX-nav: IIFE + HelpdeskPage.page. No usar DOMContentLoaded.
...
(function () {
    ...
    const RequesterSelection = {
        ...
        destroy() {
            ...
            if (this.modal) {
                try { this.modal.dispose(); } catch (e) { /* ignorar */ }
                this.modal = null;
            }
            this.selectedRequester = null;
            this.availableRequesters = [];
            this.filteredRequesters = [];
        },
    ...
    // ==================== INICIALIZACIÓN ====================
    function init() {
        // Resetear AppState al entrar (la navegación HTMX puede re-llamar init)
        AppState.currentStep = 1;
        AppState.totalSteps = 3;
        ...
        AppState.modal = null;
        AppState.groupModal = null;
        AppState.groupEquipmentModal = null;

        // Inyectar el <style> de animación de forma idempotente (guard por id)
        if (!document.getElementById('ct-anim-style')) {
            const style = document.createElement('style');
            style.id = 'ct-anim-style';
            style.textContent = `
...
`;
        ...
        // Tutorial
        window.helpdeskTutorial?.maybeAutoStart('create_ticket');
    }

    // ==================== DESTROY ====================
    function destroy() {
        // Dispose modales de Bootstrap
        if (AppState.modal) {
            try { AppState.modal.dispose(); } catch (e) { /* ignorar */ }
            AppState.modal = null;
        }
        if (AppState.groupModal) {
            try { AppState.groupModal.dispose(); } catch (e) { /* ignorar */ }
            AppState.groupModal = null;
        }
        if (AppState.groupEquipmentModal) {
            try { AppState.groupEquipmentModal.dispose(); } catch (e) { /* ignorar */ }
            AppState.groupEquipmentModal = null;
        }
        ...
        // Limpiar debug export
        delete window.CreateTicketDebug;

        // Teardown del tutorial
    ...
    const Equipment = {
        ...
        async loadGroupsForModal() {
            ...
            try {
                ...
                if (selectedRequesterId) {
                    const deptResponse = await fetch(`/api/core/v2/users/${selectedRequesterId}/department`);
                    if (!deptResponse.ok) {
                    ...
                    const userResponse = await fetch('/api/core/v2/users/me/department');
                ...
                const response = await fetch(`/api/help-desk/v2/inventory/groups/department/${deptId}`);
        ...
        async loadGroupEquipmentForModal() {
            ...
            try {
                ...
                const response = await fetch(`/api/help-desk/v2/inventory/groups/${groupId}/items`);
        ...
        async loadEquipmentForModal() {
            ...
            try {
                ...
                if (AppState.equipment.ownerType === 'mine') {
                    if (selectedRequesterId) {
                        endpoint = `/api/help-desk/v2/inventory/items/user/${selectedRequesterId}/equipment`;
                    } else {
                        endpoint = '/api/help-desk/v2/inventory/items/my-equipment';
                    }
                } else {
                    let deptId;
        ...
        clearSelection() {
            ...
            AppState.equipment.selectedGroupEquipment = [];
            const previewEl = document.getElementById('selected-equipment-preview');
            if (previewEl) previewEl.style.display = 'none';
            const previewGrpEl = document.getElementById('selected-group-equipment-preview');
            if (previewGrpEl) previewGrpEl.style.display = 'none';
            const itemIdsEl = document.getElementById('inventory_item_ids');
            if (itemIdsEl) itemIdsEl.value = '';
    ...
    const FormValidation = {
        ...
        setupDescriptionCounter() {
            ...
            // Guardar referencia para poder remover en destroy
            this._descHandler = () => {
            ...
            textarea.addEventListener('input', this._descHandler);
            this._descTextarea = textarea;
        },

        destroy() {
            if (this._descTextarea && this._descHandler) {
                this._descTextarea.removeEventListener('input', this._descHandler);
                this._descTextarea = null;
                this._descHandler = null;
            }
        },
        ...
        async submitTicket() {
            ...
            try {
                ...
                if (photoFile || Object.keys(customFieldFiles).length > 0) {
                    ...
                    const response = await fetch('/api/help-desk/v2/tickets/', {
                        method: 'POST',
                    ...
                    await HelpdeskUtils.api.createTicket(formData);
    ...
    const PhotoUpload = {
        ...
        init() {
            ...
            // Almacenar referencia al handler de paste para poder removerlo en destroy
    ...
    window.HelpdeskPage.page('user_create_ticket', { init, destroy });
