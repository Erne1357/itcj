...
def get_technician_total_time(
    ...
    from itcj2.apps.helpdesk.models import Ticket, TicketCollaborator

    # Query para tiempo como asignado principal
    lead_query = db.query(
        func.sum(Ticket.time_invested_minutes).label('total_time'),
    ...
    collab_query = db.query(
        func.sum(TicketCollaborator.time_invested_minutes).label('total_time'),
        func.count(TicketCollaborator.id).label('count')
    ).filter(
    ...
    unique_tickets_query = db.query(Ticket.id).filter(
        or_(
            Ticket.assigned_to_user_id == user_id,
            Ticket.id.in_(
                db.query(TicketCollaborator.ticket_id).filter(
                    TicketCollaborator.user_id == user_id
                )
            )
        ),
