"""
Stats API v2 — 11 endpoints (2 originales + 9 del merge feature/helpdesk-stats).
...
def _resolution_hours(t) -> Optional[float]:
    """Horas calendario entre creación y resolución."""
    if not t.resolved_at or not t.created_at:
        return None
    h = (t.resolved_at - t.created_at).total_seconds() / 3600
    return h if h >= 0 else None


def _within_sla(t) -> bool:
    """¿El ticket se resolvió dentro del SLA según prioridad?"""
    h = _resolution_hours(t)
    if h is None:
        return False
    limits = {"URGENTE": 4, "ALTA": 8, "MEDIA": 24, "BAJA": 72}
    return h <= limits.get(t.priority, 24)
...
def _iqr_bounds(values: list) -> Optional[dict]:
    if len(values) < 4:
        return None
    sv = sorted(values)
    n = len(sv)
...
def _exclude_outlier_tickets(tickets: list) -> tuple:
    res_hours = [_resolution_hours(t) for t in tickets if _resolution_hours(t) is not None]
    bounds = _iqr_bounds(res_hours)
    if not bounds:
        return tickets, None
...
def _build_base_query(
    ...
    if period_id:
        ...
        presets = {
            ...
            "3months": (now - timedelta(days=90), now),
...
def _kmeans_2d(points: list, k: int = 3, max_iter: int = 50) -> list:
    """K-means simple en 2D. Devuelve lista de asignaciones de cluster."""
    ...
    for _ in range(max_iter):
        new_assignments = []
        for p in points:
            dists = [((p[0] - c[0]) ** 2 + (p[1] - c[1]) ** 2) ** 0.5 for c in centroids]
...
def _cluster_label(centroid_h: float, centroid_r: float) -> str:
    fast = centroid_h < 8
    good = centroid_r >= 4.0
    if fast and good:
        return "Rápido y Bien Calificado"
    elif fast and not good:
        return "Rápido pero Mal Calificado"
    elif not fast and good:
        return "Lento pero Bien Calificado"
    else:
        return "Lento y Mal Calificado"
...
def get_global_stats(
    ...
    try:
        ...
        by_status = {}
        for s in ("PENDING", "ASSIGNED", "IN_PROGRESS", "RESOLVED_SUCCESS", "RESOLVED_FAILED", "CLOSED", "CANCELED"):
        ...
        invested = [t.time_invested_minutes / 60 for t in tickets if t.time_invested_minutes and t.time_invested_minutes > 0]
        avg_time_invested_hours = _safe_avg(invested)

        assign_hours = []
        for t in tickets:
            first_asgn = db.query(Assignment).filter_by(ticket_id=t.id).order_by(Assignment.assigned_at).first()
        ...
        return {
            ...
            "data": {
                ...
                "periods": _get_periods_list(db),
                "exclusion_info": exclusion_info,
            },
        }
    except Exception as e:
        logger.error(f"Error stats globales: {e}", exc_info=True)
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al obtener estadísticas globales"})
...
def get_stats_by_department(
    ...
    try:
        ...
        for t in tickets:
            ...
            dept_map.setdefault(dept_id, []).append(t)
        ...
        return {"success": True, "data": results, "exclusion_info": exclusion_info}
    except Exception as e:
        logger.error(f"Error stats por departamento: {e}", exc_info=True)
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al obtener estadísticas por departamento"})
    ...
    try:
        ...
        for tid in tech_ids:
            ...
            try:
                ...
                collab_ids = set(
                    tc.ticket_id for tc in db.query(TicketCollaborator).filter_by(user_id=tid).all()
                )
            ...
            invested = [t.time_invested_minutes / 60 for t in resolved if t.time_invested_minutes and t.time_invested_minutes > 0]
            ...
            eff_rate = round(sum(1 for t in eff_rated if t.rating_efficiency) / len(eff_rated) * 100, 1) if eff_rated else 0
...
def get_time_breakdown(
    ...
    try:
        ...
        assign_h = []
        for t in tickets:
            fa = db.query(Assignment).filter_by(ticket_id=t.id).order_by(Assignment.assigned_at).first()
            if fa and fa.assigned_at and t.created_at:
                h = (fa.assigned_at - t.created_at).total_seconds() / 3600
                if 0 <= h <= 720:
                    assign_h.append(round(h, 2))

        res_h = [round(_resolution_hours(t), 2) for t in resolved_tickets if _resolution_hours(t) is not None and _resolution_hours(t) <= 720]
        inv_h = [round(t.time_invested_minutes / 60, 2) for t in tickets if t.time_invested_minutes and t.time_invested_minutes > 0]
        ...
        for ar in ("DESARROLLO", "SOPORTE"):
            ...
            a_inv = [t.time_invested_minutes / 60 for t in a_tickets if t.time_invested_minutes and t.time_invested_minutes > 0]
            by_area.append({
                "area": ar,
                "count": len(a_tickets),
        ...
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al calcular tiempos"})
...
def get_ratings_detail(
    ...
    try:
        ...
        for t in rated:
            ...
            dept_map.setdefault(did, []).append(t)
        ...
        return {
            ...
            "data": {
                ...
                "efficiency_rate": round(sum(1 for t in eff_rated if t.rating_efficiency) / len(eff_rated) * 100, 1) if eff_rated else 0,
                "rated_count": len(rated),
        ...
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al obtener calificaciones"})
...
def get_outliers(
    ...
    try:
        ...
        res_outliers = []
        if res_bounds:
            res_outliers = [_ticket_summary(t) for t, h in res_values if h > res_bounds["upper_fence"]]
        ...
        rat_outliers = []
        if rat_bounds:
            rat_outliers = [_ticket_summary(t) for t in rated if t.rating_attention < rat_bounds["lower_fence"]]
        ...
        inv_outliers = []
        if inv_bounds:
            inv_outliers = [_ticket_summary(t) for t, h in zip(inv_tickets, inv_values) if h > inv_bounds["upper_fence"]]

        return {
            "success": True,
            "data": {
                "resolution": {"bounds": res_bounds, "outlier_count": len(res_outliers), "tickets": res_outliers[:50]},
                "rating": {"bounds": rat_bounds, "outlier_count": len(rat_outliers), "tickets": rat_outliers[:50]},
                "time_invested": {"bounds": inv_bounds, "outlier_count": len(inv_outliers), "tickets": inv_outliers[:50]},
            },
        }
    except Exception as e:
        logger.error(f"Error outliers: {e}", exc_info=True)
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al calcular outliers"})


@router.get("/analysis/kmeans")
def get_kmeans(
    period_id: Optional[int] = Query(None),
    preset: Optional[str] = Query(None),
    start: Optional[str] = Query(None),
    ...
    try:
        ...
        if len(tickets) < k:
            return {
                "success": True,
                "data": {
                    "k": k, "clusters": [], "exclusion_info": exclusion_info,
        ...
        if len(valid_tickets) < k:
            return {
                "success": True,
                "data": {"k": k, "clusters": [], "message": f"Datos válidos insuficientes ({len(valid_tickets)} tickets)."},
            }
        ...
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al ejecutar clustering"})
...
def get_distribution(
    ...
    try:
        ...
        buckets_res = [(0, 2, "<2h"), (2, 8, "2-8h"), (8, 24, "8-24h"), (24, 72, "1-3d"), (72, 168, "3-7d"), (168, 9999, ">7d")]
        ...
        buckets_inv = [(0, 0.5, "<30min"), (0.5, 2, "30min-2h"), (2, 8, "2-8h"), (8, 24, "8-24h"), (24, 9999, ">24h")]
        inv_h_list = [t.time_invested_minutes / 60 for t in tickets if t.time_invested_minutes and t.time_invested_minutes > 0]
        ...
        by_weekday = [
            {"day": DAYS[i], "day_idx": i, "count": sum(1 for t in tickets if t.created_at and t.created_at.weekday() == i)}
            for i in range(7)
        ]

        by_hour = [
            {"hour": h, "label": f"{h:02d}:00", "count": sum(1 for t in tickets if t.created_at and t.created_at.hour == h)}
            for h in range(24)
        ]
        ...
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al calcular distribuciones"})
    ...
    try:
        ...
        for i in range(23, -1, -1):
            ...
            monthly.append({
                ...
                "sla_rate": round(sum(1 for t in m_resolved if _within_sla(t)) / len(m_resolved) * 100, 1) if m_resolved else 0,
            })
        ...
        yoy = []
        for m in range(1, 13):
            this_y_count = sum(1 for t in tickets if t.created_at and t.created_at.year == this_year and t.created_at.month == m)
            prev_y_count = sum(1 for t in tickets if t.created_at and t.created_at.year == prev_year and t.created_at.month == m)
            yoy.append({
                "month": m,
        ...
        for dow in range(7):
            for h in range(24):
                count = sum(1 for t in recent if t.created_at and t.created_at.weekday() == dow and t.created_at.hour == h)
        ...
        raise HTTPException(500, detail={"error": "server_error", "message": "Error al calcular tendencias"})
