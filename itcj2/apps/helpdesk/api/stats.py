...
def get_technician_stats(
    ...
    rated_tickets = Ticket.query.filter_by(assigned_to_user_id=user_id).filter(Ticket.rating_attention.isnot(None)).all()
    satisfaction_percent = 0
    if rated_tickets:
        avg_rating = sum(t.rating_attention for t in rated_tickets) / len(rated_tickets)
        satisfaction_percent = round((avg_rating / 5) * 100, 1)
    ...
    resolved_today_count = Ticket.query.filter_by(assigned_to_user_id=user_id).filter(
