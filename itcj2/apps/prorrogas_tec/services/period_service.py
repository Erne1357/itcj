"""Períodos de prórrogas: ventana de admisión y configuración por período.

Vivía en `itcj2/core/services/prorrogas_period_service.py`, pero importa
`ProrrogasPeriodConfig` de esta app: core dependiendo de una app es la
inversión de capas que el gotcha #2 de CLAUDE.md señala como fuente de
imports circulares. Vive donde vive su modelo.
"""
...
def get_active_period(db: Session) -> Optional[ProrrogasPeriodConfig]:

    now = datetime.now()
    print(f"VALOR DE NOW: {now} (Tipo: {type(now)})")
    
    return (
            db.query(ProrrogasPeriodConfig)
            .filter(
                ProrrogasPeriodConfig.student_admission_start <= now,
                ProrrogasPeriodConfig.student_admission_deadline >= now,
            )
            .first()
...
def get_period_by_id(db: Session, period_id: int) -> Optional[ProrrogasPeriodConfig]:
    return db.get(ProrrogasPeriodConfig, period_id)
...
def is_student_window_open(db: Session) -> bool:
    ...
    if period:
        ...
        print(f"Inicio: {period.student_admission_start}")
        print(f"Fin: {period.student_admission_deadline}")
    else:
        print("No existe un periodo activo")
        
    if not period:
        return False
    config = get_prorrogas_config(db, period.period_id)
    if not config:
        return False
...
def get_prorrogas_config(db: Session, period_id: Optional[int] = None):
    from itcj2.apps.prorrogas_tec.models.prorrogas_period_config import ProrrogasPeriodConfig
    ...
    return db.query(ProrrogasPeriodConfig).filter_by(period_id=period_id).first()
