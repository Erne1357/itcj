(() => {
  ...
  let enabledDays = []; // Se carga desde /api/agendatec/v2/periods/active
  ...
  programSelect.addEventListener("change", async (e) => {
    ...
    try {
      const r = await fetch(`/api/agendatec/v2/programs/${state.program_id}/coordinator`, { credentials: "include" });
      if (!r.ok) throw 0;
  ...
  async function loadSlots() {
    ...
    try {
      const r = await fetch(`/api/agendatec/v2/availability/program/${state.program_id}/slots?day=${state.day}`, {
        credentials: "include"
      });
      if (!r.ok) throw 0;
      const data = await r.json();
