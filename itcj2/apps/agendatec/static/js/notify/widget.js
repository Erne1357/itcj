...
(() => {
    ...
    async function fetchUnreadCount() {
        // trae pocas y cuenta
        const r = await fetch("/api/agendatec/v2/notifications?unread=1&limit=50", { credentials: "include" });
    ...
    async function loadRecent() {
        const r = await fetch("/api/agendatec/v2/notifications?limit=8", { credentials: "include" });
        const data = await r.json();
        list.innerHTML = (data.items || []).map(itemHTML).join("") || `<div class="text-muted px-2 py-1">Sin notificaciones.</div>`;
    }

    async function loadHistory(reset = false) {
        let url = "/api/agendatec/v2/notifications?limit=20";
    ...
    btnMarkAll?.addEventListener("click", async () => {
        await fetch("/api/agendatec/v2/notifications/read-all", { method: "PATCH", credentials: "include" });
        setBadge(0);
        // marca visualmente
