...
def test_hay_25_modelos_declarados():
    ...
    assert len(declared) == 25, (
        f"se esperaban 25 modelos titulatec, el AST vio {len(declared)}: "
        f"{sorted(declared)}"
    )
