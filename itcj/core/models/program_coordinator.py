...
class ProgramCoordinator(db.Model):
    ...
    program_id = db.Column(db.Integer, db.ForeignKey("core_programs.id", onupdate="CASCADE", ondelete="CASCADE"), primary_key=True)
    coordinator_id = db.Column(db.Integer, db.ForeignKey("core_coordinators.id", onupdate="CASCADE", ondelete="CASCADE"), primary_key=True)
