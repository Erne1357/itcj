...
# ---------------------------------------------------------------------------
# Helpers de fecha y filtros
# ---------------------------------------------------------------------------

def _parse_filters():
    """Extrae y valida los parámetros de filtro de la request."""
    period_id  = request.args.get('period_id', type=int)
    preset     = request.args.get('preset', '')        # today|week|month|year|prev_year
    start_raw  = request.args.get('start_date', '')
    end_raw    = request.args.get('end_date', '')
    area       = request.args.get('area', '')           # DESARROLLO|SOPORTE
    return period_id, preset, start_raw, end_raw, area


def _apply_date_filter(query, preset, start_raw, end_raw, period_id):
    """
    Aplica un filtro de fecha a Ticket.created_at.
    Precedencia: period_id > preset > start/end personalizados
    """
    now = datetime.utcnow()
    ...
    if period_id:
        try:
            ...
            if period:
                ...
                return query.filter(Ticket.created_at.between(start_dt, end_dt))
        except Exception:
            pass  # Si no existe el modelo, ignorar silenciosamente

    if preset == 'today':
        d = now.date()
        return query.filter(Ticket.created_at >= datetime.combine(d, datetime.min.time()))
    elif preset == 'week':
        start = now - timedelta(days=now.weekday())
        return query.filter(Ticket.created_at >= start.replace(hour=0, minute=0, second=0))
    elif preset == 'month':
        start = now.replace(day=1, hour=0, minute=0, second=0)
        return query.filter(Ticket.created_at >= start)
    elif preset == 'year':
        start = now.replace(month=1, day=1, hour=0, minute=0, second=0)
        return query.filter(Ticket.created_at >= start)
    elif preset == 'prev_year':
        prev = now.year - 1
        start = datetime(prev, 1, 1)
        end   = datetime(prev, 12, 31, 23, 59, 59)
        return query.filter(Ticket.created_at.between(start, end))
...
def _base_query(period_id, preset, start_raw, end_raw, area):
    """Construye la query base de Ticket con todos los filtros aplicados."""
    q = Ticket.query
    q = _apply_date_filter(q, preset, start_raw, end_raw, period_id)
...
def _resolution_hours(ticket):
    if ticket.resolved_at and ticket.created_at:
        return (ticket.resolved_at - ticket.created_at).total_seconds() / 3600
    return None


def _within_sla(ticket):
    """True si el ticket fue resuelto dentro del SLA."""
    SLA = {'URGENTE': 4, 'ALTA': 24, 'MEDIA': 72, 'BAJA': 168}
    h = _resolution_hours(ticket)
    if h is None:
        return False
    limit = SLA.get(ticket.priority, 72)
    return h <= limit


def _get_periods_list():
    """Retorna lista de períodos para los filtros del frontend."""
    try:
        from itcj.core.models.academic_period import AcademicPeriod
...
def _iqr_bounds(values):
    """Calcula Q1, Q3 y las vallas IQR para detección de outliers."""
    if len(values) < 4:
        return None
    s = sorted(values)
    n = len(s)
...
def _kmeans_2d(points, k=3, max_iter=100, seed=42):
    ...
    Retorna lista de asignaciones (0..k-1) de cada punto.
    """
    ...
    max_x = max(xs) or 1.0
    max_y = max(ys) or 1.0
    norm = [(x / max_x, y / max_y) for x, y in points]

    # Inicializar centroides (k-means++)
    centroids = [norm[rng.randrange(len(norm))]]
    ...
    for _ in range(max_iter):
        new_asgn = []
        for p in norm:
            best = min(range(k), key=lambda j: (p[0]-centroids[j][0])**2 + (p[1]-centroids[j][1])**2)
            new_asgn.append(best)
        ...
        new_centroids = []
        for j in range(k):
            cluster_pts = [norm[i] for i, a in enumerate(assignments) if a == j]
            if cluster_pts:
...
def _cluster_label(centroid_hours, centroid_rating):
    """Genera una etiqueta descriptiva para un clúster según su centroide."""
    fast = centroid_hours < 4
    slow = centroid_hours > 24
    good = centroid_rating >= 4.0
    poor = centroid_rating < 3.0

    if fast and good:
        return 'Rápido y bien valorado'
    elif fast and poor:
        return 'Rápido pero mal valorado'
    elif slow and good:
        return 'Lento pero bien valorado'
    elif slow and poor:
        return 'Lento y mal valorado'
    elif good:
        return 'Bien valorado'
    elif poor:
        return 'Mal valorado'
    else:
        return 'Promedio'
